...
// -----------------------------------------------------------
	...
	public:
		Scene()
		{
...
#endif
			sphere = Sphere(++objIdx, float3(0), 0.6f);				// 1: bouncing ball
			sphere2 = Sphere(++objIdx, float3(0, 2.5f, -3.07f), 8);	// 2: rounded corners
			cube = Cube(++objIdx, float3(0), float3(1.15f));			// 3: cube
			...
			torus = Torus(++objIdx, 0.8f, 0.25f);						// 10: torus
